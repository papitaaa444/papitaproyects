...
class RutaCarta:
    def __init__(self, root):
        self.root = root
        self.root.title("💌 Ruta hacia la carta")
    ...
    def estilo_label(self, texto, tamaño=14, negrita=False, color=None, bg=None):
        return tk.Label(
            self.frame, text=texto,
            bg=bg or PALETA["bg"],
            fg=color or PALETA["texto"],
            font=("Helvetica", tamaño, "bold" if negrita else "normal"),
            pady=5
        )
    ...
    def juego1(self):
        self.limpiar()
        self.titulo("🎮 Juego 1: Quiz de recuerdos").pack(pady=10)

        preguntas = [
            ("¿Cuál fue nuestro primer plan juntos?", ["cine", "cafe", "sushi"], "sushi"),
            ("¿Qué color te recuerda a mí?", ["azul", "rosado", "negro"], "rosado"),
            ("¿Qué es lo que más hacemos?", ["leer", "viajar", "jugar"], "jugar")
        ]
    ...
    def verificar_quiz(self):
        ...
        if score == 3:
            self.resultado.config(text="✅ ¡Perfecto! Avanza cuando quieras.", fg="green")
            self.estilo_boton("Siguiente ➡️", self.juego2).pack(pady=10)
        else:
            self.resultado.config(text=f"❌ Fallaste ({score}/3). Intenta de nuevo.", fg="#b00020")

    # --- Juego 2: Atrapa corazones ---
    def juego2(self):
        self.limpiar()
        self.titulo("🎮 Juego 2: Atrapa corazones ❤️").pack(pady=4)
        ...
        self.estilo_boton("Iniciar ➡️", self.iniciar_corazones).pack(in_=contenedor_boton, side="left", padx=6)
        self.btn_detener = self.estilo_boton("Detener ⏸️", self.detener_corazones, color="#9c36b5")
    ...
    def spawner(self):
        ...
        if self.caught >= self.target:
            if not self.next_btn_rendered:
                self.estilo_boton("Siguiente ➡️", self.juego3).pack(pady=10)
        ...
        heart = self.canvas.create_text(x, y, text="❤️", font=("Helvetica", size))
    ...
    def catch_heart(self, heart):
        try:
            self.canvas.itemconfig(heart, text="💖")
    ...
    def juego3(self):
        self.limpiar()
        self.titulo("🎮 Juego 3: Adivinanza").pack(pady=10)

        texto_adivinanza = (
            "Viajo entre estrellas, un niño de cabellos dorados,\n"
            "busco la amistad, a mis amigos he domesticado.\n"
            "Lo importante no se ve, se siente con el alma,\n"
            "¿Quién soy, que te enseño el amor sin calma?"
        )
    ...
    def verificar_riddle(self):
        if self.entry.get().strip().lower() == "el principito":
            self.riddle_result.config(text="✅ ¡Correcto!", fg="green")
            self.estilo_boton("Siguiente ➡️", self.juego4).pack(pady=10)
        else:
            self.riddle_result.config(text="❌ No es correcto. Pista: El Principito", fg="#b00020")

    # --- Juego 4: Memoria de parejas ---
    def juego4(self):
        self.limpiar()
        self.titulo("🎮 Juego 4: Memoria de corazones").pack(pady=10)
        ...
        grid.pack(pady=6)
        for i in range(6):
            b = tk.Button(grid, text="❓", width=6, height=3, font=("Helvetica", 14),
    ...
    def reveal(self, i):
        if i in self.selected or self.buttons[i]["text"] != "❓":
            return
        ...
        if len(self.selected) == 2:
            ...
            if self.values[a] == self.values[b]:
                ...
                if self.matched == 3:
                    self.estilo_boton("Siguiente ➡️", self.juego5).pack(pady=10)
            else:
                def hide_pair(x=a, y=b):
                    self.buttons[x].config(text="❓")
                    self.buttons[y].config(text="❓")
                    self.selected = []
                self.root.after(800, hide_pair)

    # --- Juego 5: Trivia de Fútbol personalizada (con Siguiente) ---
    def juego5(self):
        self.limpiar()
        self.titulo("🎮 Juego 5: Trivia de Fútbol").pack(pady=10)

        preguntas = [
            ("¿Qué selección ganó el Mundial de 2014?", ["Alemania", "Brasil", "Argentina"], "Alemania"),
            ("¿Quién salió como mejor jugador de Chile en 2025?",
             ["Alexis Sánchez", "Arturo Vidal", "Charles Aránguiz"], None),  # sin correcta
            ("¿Cuántos jugadores tiene un equipo en cancha?", ["9", "10", "11"], "11")
        ]
    ...
    def check_trivia(self):
        ...
        mensaje_nota = (
            "⭐ Nota especial: El mejor futbolista de este año serías tú Kevin Encina,\n"
            "porque lamentablemente no te conocí antes :3,\n"
            "pero desde ahora lo eres y en adelante lo serás a no ser que te alejes de mí,\n"
            "ahí dejarás de serlo, pero como no pasará, no hay por qué pensar en eso :b."
        )
        self.trivia_resultado.config(text=mensaje_nota, fg=PALETA["acento"])

        # Mostrar botón Siguiente solo si las otras dos (válidas) están correctas
        if score == total - 1:
            if self.btn_siguiente is None or not self.btn_siguiente.winfo_exists():
                self.btn_siguiente = self.estilo_boton("Siguiente ➡️", self.juego6)
                self.btn_siguiente.pack(pady=10)
        else:
            self.trivia_resultado.config(
                text=(
                    f"❌ Fallaste ({score}/{total - 1} válidas). Corrige las otras preguntas.\n\n" +
                    mensaje_nota
                ),
                fg="#b00020"
            )
    ...
    # --- Juego 6: Adivina el número ---
    def juego6(self):
        self.limpiar()
        self.titulo("🎮 Juego 6: Adivina el número").pack(pady=10)

        self.secret = random.randint(1, 10)
        self.subtitulo("He pensado un número entre 1 y 10").pack()
    ...
    def check_number(self):
        ...
        try:
            ...
            self.num_result.config(text="❌ Ingresa un número válido (1–10)", fg="#b00020")
            return
        if not (1 <= guess <= 10):
            self.num_result.config(text="❌ Debe estar entre 1 y 10", fg="#b00020")
            return
        ...
        if guess < self.secret:
            self.num_result.config(text="El número es más alto ⬆️", fg=PALETA["acento"])
        else:
            self.num_result.config(text="El número es más bajo ⬇️", fg=PALETA["acento"])
        self.info_intentos.config(text=f"Intentos restantes: {self.intentos}")

    # --- Carta final ---
    def carta(self):
        self.limpiar()
        self.titulo("💌 Carta desbloqueada 💌").pack(pady=10)

        texto = (
            "Gracias por jugar y llegar hasta aquí.\n"
            "Cada corazón atrapado es un recuerdo, pude poner más de 7 pero seguirías atrapando corazones ya que son tantos.\n"
            "Pero bueno, te dejo esta pequeña carta virtual como fin de este código, es muy simple y sincera:\n"
            "Me importas, me inspiras y me haces querer ser mejor y no solo eso, de verdad me haces sentir tanto,\n"
            "que llega a darme miedo porque ahora que te conocí no quiero perderte nunca.\n"
            "Muchas gracias por tus detalles, de verdad los valoro un montón y me hacen muy feliz.\n"
            "Espero que este pequeño detalle también te haga feliz como tú lo haces conmigo.\n\n"
            "Byebye con cariño,\nScarlett ❤️\n\n"
        )
    ...
    def animar_corazones(self, canvas):
        ...
        heart = canvas.create_text(x, 320, text="❤️", font=("Helvetica", size))
        def mover():
            try:
